...
export async function* traverseGroupIterationRecursiveCall({
  ...
  */
  ...
  for await (let { group } of groupIterator) {
    ...
    */
    let traversalIterator = this::this.traversalIteration({ nodeIteratorFeed: group.nodeIterator, additionalChildNode, traverserPosition, traverseCallContext })
    let traversalPromiseFeed = await this::this.traverseIterationRecursiveCall({ traversalIterator })
...
*/
export async function* traversalIteration({ nodeIteratorFeed, additionalChildNode, traverserPosition, traverseCallContext, traverser = this }) {
  ...
  while (!iteratorObject.done) {
    yield {
      ...
      traversalInvocation: nextFunction =>
        traverser::traverser.traverse({ nodeInstance: iteratorObject.value.node /* next node */, additionalChildNode }, { parentTraverserPosition: traverserPosition, traverseCallContext }),
    }
...
export async function* traverseIterationRecursiveCall({ traversalIterator, traverser = this }) {
  ...
  while (!iteratorObject.done) {
    yield { traversalPromise: iteratorObject.value.traversalInvocation() }

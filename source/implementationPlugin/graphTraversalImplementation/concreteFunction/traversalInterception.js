...
export const handleMiddlewareNextCall = ({ dataProcessCallback, targetFunction, aggregator }) => {
  return new Proxy(targetFunction, {
    async apply(target, thisArg, argArray) {
      ...
      eventEmitter.on('nodeTraversalCompleted', data => null /* console.log(data.value, ' resolved.') */)
  ...
  return new Proxy(targetFunction, {
    async apply(target, thisArg, argArray) {
      ...
      if (conditionMet) {
        ...
        await dataProcessCallback({ nextProcessData: aggregator.value, additionalParameter: {} })

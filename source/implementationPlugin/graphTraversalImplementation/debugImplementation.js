...
export function aggregateIntoArray({ thisArg }) {
      ...
      let implementationObject = {
        ...
        async chronological() {
          let array = await nodeConnectionArray.reduce(async (accumulatorPromise, nodeConnection, index) => {
            // sequential accumolation of results
            ...
            // cocatinate arrays
            accumulatorArray.length != 0 ? Array.prototype.push.apply(accumulatorArray, subsequentArray) : (accumulatorArray = subsequentArray.slice()) // prevents error when acculator is not an array.

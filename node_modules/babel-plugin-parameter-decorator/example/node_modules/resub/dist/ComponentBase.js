...
var ComponentBase = /** @class */ (function (_super) {
    ...
    ComponentBase.prototype._addSubscription = function (subscription) {
        utils_1.assert(subscription.store instanceof StoreBase_1.StoreBase, "Subscription added with store that's not an StoreBase");
    ...
    ComponentBase.prototype._registerSubscription = function (subscription, key) {
        ...
        utils_1.assert(!subscription._subscriptionToken, 'Subscription already subscribed!');
        utils_1.assert(!subscription.keyPropertyName || key !== StoreBase_1.StoreBase.Key_All, 'Subscription created with key of all when it has a key property name');
        utils_1.assert(!_.isEqual(subscription.specificKeyValue, StoreBase_1.StoreBase.Key_All), 'Subscription created with specific key of all');
        if (key) {
            key = utils_1.normalizeKey(key);
    ...
    ComponentBase.prototype._findKeyFromPropertyName = function (props, keyPropertyName) {
        ...
        if (!_.isString(key)) {
            utils_1.assert(false, "Subscription key property value " + keyPropertyName + " must be a string");
    ...
    ComponentBase.prototype._buildInitialState = function () {
        ...
        // Initialize state
        var initialState = this._buildStateWithAutoSubscriptions(this.props, true) || {};
        return initialState;
    };

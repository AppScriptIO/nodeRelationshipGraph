...
function enableAutoSubscribe(handler) {
    return function (target, propertyKey, descriptor) {
        ...
        utils_1.assert(_.isFunction(existingMethod), 'Can only use @enableAutoSubscribe on methods');
...
function makeAutoSubscribeDecorator(shallow, autoSubscribeKeys) {
    ...
    return function (target, methodName, descriptor) {
        ...
        // Note: T might have other properties (e.g. T = { (): void; bar: number; }). We don't support that and need a cast/assert.
        var existingMethod = descriptor.value;
        utils_1.assert(_.isFunction(existingMethod), 'Can only use @autoSubscribe on methods');
        // Note: we need to be given 'this', so cannot use '=>' syntax.
        descriptor.value = function AutoSubscribe() {
            ...
            if (metaForMethod.keyIndexes) {
                keyParamValues = metaForMethod.keyIndexes.map(function (index) {
                    ...
                    utils_1.assert(_.isString(keyArg), "@key parameter must be given a string or number: " +
            ...
            var specificKeyValues = (autoSubscribeKeys && autoSubscribeKeys.length > 0) ?
                _.map(autoSubscribeKeys, function (autoSubKey) { return utils_1.formCompoundKey.apply(void 0, keyParamValues.concat(autoSubKey)); }) :
                [(keyParamValues.length > 0) ? utils_1.formCompoundKey.apply(void 0, keyParamValues) : StoreBase_1.StoreBase.Key_All];
...
function autoSubscribeWithKey(keyOrKeys) {
    utils_1.assert(keyOrKeys || _.isNumber(keyOrKeys), 'Must specify a key when using autoSubscribeWithKey');
